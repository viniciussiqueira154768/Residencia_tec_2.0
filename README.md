...
<div style="text-align: center;">
  <h1>💻 WebPower - Sistema de Ponto Web 🧭</h1>
</div>
...
O WebPower é uma Single Page Application (SPA) de gestão de frequência, desenvolvida para modernizar a maneira como colaboradores e gestores lidam com o registro de ponto. Utilizando o poder do React e JavaScript, o projeto se destaca pela sua interface intuitiva e pela centralização de processos de RH, como gestão de férias e justificativas de ausência. O sistema garante confiabilidade e transparência ao integrar recursos como a visualização da localização geográfica no momento do ponto. 
...
- `Bater Ponto com Validação Visual`: 
     - Registro de entrada, saída e intervalos em uma única tela.

     - Diferencial: Exibe um mapa com a geolocalização atual, confirmando visualmente o local do registro para maior transparência e segurança.

- `Justificativa de Ausência`: 
     - Formulário dedicado e calendário interativo para selecionar o     período e anexar documentos ou descrever a justificativa (ex: atestado, atraso).

- `Histórico Detalhado`: 
    - Permite a consulta e filtragem dos registros de ponto por períodos específicos (mês atual, meses anteriores).

- `Dashboard de Comparecimento (Insights)`: 
    - Apresenta um resumo visual do desempenho, utilizando gráficos de barras para mostrar a frequência ou horas trabalhadas, além de indicadores rápidos de saldo de horas.

- `Gestão de Férias`:
    - Tela completa para solicitação de novos períodos de férias.

    - Diferencial: Exibição de um calendário visual para acompanhamento dos dias já solicitados ou disponíveis, juntamente com o histórico completo de férias passadas.
...
Para clonar e executar o WebPower em sua máquina, siga os passos abaixo:
...
```bash
# Exemplo (Verifique o arquivo .env.example para as variáveis corretas)
REACT_APP_API_URL=http://localhost:3001/api
```
4. **Iniciar o Aplicativo**
Execute o comando de start do React. A aplicação será iniciada em modo de desenvolvimento.
...
A aplicação deverá ser aberta automaticamente no seu navegador em **http://localhost:3000.**
...
Para Execução e Dependências (Ambiente de Desenvolvimento):
...
Agradecimento especial aos nossos mentores que contribuíram indiretamente com feedback e direcionamento técnico.
